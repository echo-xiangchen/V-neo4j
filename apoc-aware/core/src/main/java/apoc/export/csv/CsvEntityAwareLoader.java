...
import apoc.expr.composite.Expr;
...
public class CsvEntityAwareLoader {

    private final CsvLoaderConfig clc;
    private final ProgressReporter reporter;
    private final Log log;
    // this map is for storing the <pc,BDD> map, so that for the same string of pc, don't need to parse it multiple times
    private final static Map<String, Long> BddMap = new LinkedHashMap<String, Long>();

    /**
     * @param clc configuration object
     * @param reporter
     */
    public CsvEntityAwareLoader (CsvLoaderConfig clc, ProgressReporter reporter, Log log) {
        this.clc = clc;
    ...
    public void loadNodes(final String fileName, final List<String> labels, final GraphDatabaseService db,
                          final Map<String, Map<String, Long>> idMapping, final Antlr2Expr antlr2Expr, 
                          final BDDbuilder bddBuilder, CsvHeaderField bdd) throws IOException {
        final CountingReader reader = FileUtils.readerFor(fileName);
        // header is the first line of the input file, e.g., :ID|name:STRING|speaks:STRING[]
        final String header = readFirstLine(reader);
        // clc.getSkipLines() default value is 1
        reader.skip(clc.getSkipLines() - 1);
        // fields simply separate the header into different sections based on the delimiter
        // e.g., fields[0].name = __csv_id, fields[0].type = ID
        // fields[1].name = name, fields[1].type = STRING
        // fields[2].name = speaks, fields[2].type = STRING
        ...
        // if inside the node .csv file, the :ID column is not provided:
        ...
        try (BatchTransaction btx = new BatchTransaction(db, clc.getBatchSize(), reporter)) {
            for (String[] line : csv.readAll()) {
                ...
                // add labels
                /**
                 * Need to check if the list of labels is empty
                 * skip if use :LABEL to specify labels inside .csv file
                 */
                if (!labels.isEmpty()) {
                ...
                for (CsvHeaderField field : fields) {
                    ...
                    if (field.isMeta()) {
                        ...
                        // check if it's the pc property
                        if (name.equals("pc")) {
    						// check if BDDexprMap already contains the presence condition
                        	// if yes, then don't parse it again - memory saving
                        	if (!BddMap.containsKey(value)) {
    							parsePC((String) value, antlr2Expr, bddBuilder);
    						}
                        	// link the BDD to the node
                        	boolean bddPropertyAdded = CsvPropertyConverter.addPropertyToGraphEntity(node, bdd, BddMap.get(value), clc);
                            props += bddPropertyAdded ? 1 : 0;
    					}
                    }
                }
     ...
     * @param antlr2Expr a parser that parse a string into Expr hierarchy
     * @param bddBuilder visitor class that generates a BDD from an Expr hierarchy
     * @param bdd a CsvHeaderField that used for create an field for storing the BDD and link it to the specific edge
     * @param input,lexer,tokens,parser,tree are all Antlr classes for parsing a string
     * @throws IOException
     */
    @SuppressWarnings("deprecation")
	public void loadRelationships(
            final String fileName, final String type, final GraphDatabaseService db,
            final Map<String, Map<String, Long>> idMapping, final Antlr2Expr antlr2Expr, final BDDbuilder bddBuilder,
            CsvHeaderField bdd) throws IOException {
        final CountingReader reader = FileUtils.readerFor(fileName);
        // header is the first line of the relationship file
        // e.g., :START_ID|:END_ID|since:INT|pc
        final String header = readFirstLine(reader);
        // fields are a list of headers, each element in the list is a header of one column
        // e.g., fields[0].name = "__csv_start_id", fields[0].type = "START_ID"
        // 		 fields[1].name = "__csv_end_id", fields[1].type = "END_ID"
        // 		 fields[2].name = "since", fields[2].type = "INT"
        final List<CsvHeaderField> fields = CsvHeaderFields.processHeader(header, clc.getDelimiter(), clc.getQuotationCharacter());
            ...
            for (String[] line : csv.readAll()) {
                ...
                for (CsvHeaderField field : edgePropertiesFields) {
                    ...
                    if (name.equals("pc")) {
                    	...
                    	// link the BDD to the edge
                    	boolean bddPropertyAdded = CsvPropertyConverter.addPropertyToGraphEntity(rel, bdd, BddMap.get(value), clc);
                        props += bddPropertyAdded ? 1 : 0;
					}
                }
     ...
     * @throws IOException
     */
    public void addNode (final List<String> labels, final Map<String, Object> properties, final GraphDatabaseService db) {
    	try (BatchTransaction btx = new BatchTransaction(db, clc.getBatchSize(), reporter)) {
    		// create node
    		final Node node = btx.getTransaction().createNode();
     ...
     */
    @SuppressWarnings("deprecation")
	private void parsePC(String pc, Antlr2Expr antlr2Expr, BDDbuilder bddBuilder) {
