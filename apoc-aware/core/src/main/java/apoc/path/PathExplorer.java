...
public class PathExplorer {
	...
	@Procedure("apoc.path.extend")
	@Description("apoc.path.extend(startPath <id>|Node|list, {minLevel,maxLevel,uniqueness,relationshipFilter,labelFilter,uniqueness:'RELATIONSHIP_PATH',bfs:true, filterStartNode:false, limit:-1, optional:false, endNodes:[], terminatorNodes:[], sequence, beginSequenceAtStart:true}) yield path - " +
			"expand from start node following the given relationships from min to max-level adhering to the label filters. ")
	public Stream<PathResult> extendConfig(@Name("start") Path startPath, @Name("config") Map<String,Object> config) throws Exception {
	    ...
	    return expandConfigPrivate(startNode, config)
	            .map(path -> combine(startPath, path))  // concatenate startPath and each result path
	            .map(PathResult::new);  // convert each concatenated path to a PathResult object and return a stream
	}
	
	@Procedure("ICBC.filterComp")
	@Description("ICBC.filterComp(startNode <id>|Node|list, {minLevel,maxLevel,uniqueness,relationshipFilter,labelFilter,uniqueness:'RELATIONSHIP_PATH',bfs:true, filterStartNode:false, limit:-1, optional:false, endNodes:[], terminatorNodes:[], sequence, beginSequenceAtStart:true}) yield path - " +
			"expand from start node following the given relationships from min to max-level adhering to the label filters. ")
	public Stream<PathResult> ICBCfilterComp(@Name("start") Object start, @Name("config") Map<String,Object> config) throws Exception {
		...
		Stream<Path> comp3 = expandConfigPrivate(start, config3).filter(filter3comp);
		Stream<Path> comp4 = expandConfigPrivate(start, config4).filter(filter4comp);
		
		// Combine the results
        Stream<Path> combinedResults = Stream.concat(comp3, comp4);

        // Filter the combined results and map to PathResult
        return combinedResults.map(PathResult::new);
	}
	
	@Procedure("BA.filterComp")
	@Description("BA.filterComp(startNode <id>|Node|list, {minLevel,maxLevel,uniqueness,relationshipFilter,labelFilter,uniqueness:'RELATIONSHIP_PATH',bfs:true, filterStartNode:false, limit:-1, optional:false, endNodes:[], terminatorNodes:[], sequence, beginSequenceAtStart:true}) yield path - " +
			"expand from start node following the given relationships from min to max-level adhering to the label filters. ")
	public Stream<PathResult> BAfilterComp(@Name("start") Object start, @Name("config") Map<String,Object> config) throws Exception {
		Map<String,Object> config3 = new HashMap<>();
		...
		return expandConfigPrivate(start, config3).filter(filter3comp).map(PathResult::new);
